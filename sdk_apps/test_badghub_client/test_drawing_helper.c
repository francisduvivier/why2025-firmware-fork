...
void draw_text(SDL_Renderer *fb, char const *text, int x, int y, uint32_t color) {
    // uint16_t color565  = rgb888_to_rgb565(color, color.g, color.b);
    int current_x = x;
    int current_y = y;
    ...
    for (int i = 0; text[i];) {
        ...
        if ((unsigned char)text[i] >= 0x80) {
            ...
            if (special_idx >= 0) {
                draw_special_symbol(fb, special_idx, current_x, current_y, color);
            } else {
                // Fallback
                draw_char(fb, '?', current_x, current_y, color);
            }
            ...
            draw_char(fb, text[i], current_x, current_y, color);
            current_x += 8;
            i++;
        }
    }
